...
const Formulaire = () => {
  ...
  const ajouterArticle = () => {
    setArticles([...articles, { ref: "", description: "", quantite: '', prixUnitaire: '', total: '' }]);
  };
    ...
    const updatedArticles = articles.map((article, i) => {
      if (i === index) {
        ...
        if (e.target.name === 'quantite' || e.target.name === 'prixUnitaire') {
          const quantite = e.target.name === 'quantite' ? Number(e.target.value) : Number(article.quantite);
          const prixUnitaire = e.target.name === 'prixUnitaire' ? Number(e.target.value) : Number(article.prixUnitaire);
  ...
  const handleSubmit = async (e) => {
    ...
    const clientSelectionne = clients.find(client => client._id === selectedClientId);
    console.log("Client sélectionné:", clientSelectionne); // Pour diagnostic
  
    const invoiceData = {
      Client: clientSelectionne ? clientSelectionne._id : null,
      client: {
        name: clientSelectionne ? clientSelectionne.name : e.target.fullName.value,
         address: clientSelectionne ? clientSelectionne.address : e.target.address.value,
        email: clientSelectionne ? clientSelectionne.email : e.target.emailAddress.value,
        telephone: clientSelectionne ? clientSelectionne.telephone : e.target.phoneNumber.value,
      },
      ...
      total: articles.reduce((acc, article) => acc + (Number(article.quantite) * Number(article.prixUnitaire)), 0),
