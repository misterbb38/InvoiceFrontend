...
function EditClientButton({ clientId, onClientUpdated }) {
  ...
  const renderError = (fieldName) => {
    if (formErrors[fieldName]) {
      return <span className="text-red-500 text-xs">{formErrors[fieldName]}</span>;
    }
    return null;
  };
  ...
  return (
    <>
      <button className="btn btn-secondary" onClick={() => setShowModal(true)}>Éditer</button>
      {showModal && (
          ...
          <div className="modal-box">
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-control">
                ...
                <input className="input input-bordered" type="text" name="address" value={formData.address} onChange={handleChange} />
                {renderError('address')}
              </div>
              <div className="form-control">
                <label className="label">Email</label>
                <input className="input input-bordered" type="email" name="email" value={formData.email} onChange={handleChange} />
                {renderError('email')}
              </div>
              <div className="form-control">
                <label className="label">Téléphone</label>
                <input className="input input-bordered" type="text" name="telephone" value={formData.telephone} onChange={handleChange} />
