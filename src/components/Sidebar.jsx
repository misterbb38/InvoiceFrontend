     ...
     <div className="hidden md:flex w-64 bg-base-300 base-content flex-col">
      ...
      <ul className="mt-4 w-full">
        {menuItems.map((menuItem) => (
          <li key={menuItem.id} className={`p-2 mt-4 cursor-pointer flex justify-center items-center text-center font-bold text-lg ${selectedMenuItem === menuItem.id ? 'bg-primary text-white'  : ''}`} onClick={() => handleMenuItemClick(menuItem.id)}>
            <Link to={menuItem.route} className="focus:outline-none w-full flex items-left justify-left">
