...
function GenerateAccessKey() {
  ...
  const handleSubmit = async (e) => {
    ...
    const userInfo = JSON.parse(localStorage.getItem("userInfo")) || {};
      ...
      const data = await response.json();
      if (response.ok) {
        showToast(`Clé d'accès générée avec succès pour l'utilisateur`, 'success');
      } else {
        throw new Error(data.message || "Erreur lors de la génération de la clé d'accès.");
      }
    } catch (error) {
      showToast(error.message, 'error');
    } finally {
    ...
    <div className="max-w-4xl mx-auto my-10 p-5 bg-white rounded-lg shadow">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="form-group">
          <label htmlFor="userEmail" className="form-label inline-block mb-2 text-gray-700">Email des utilisateurs</label>
          <select
            id="userEmail"
        ...
        <div className="form-group">
          <label htmlFor="duree" className="form-label inline-block mb-2 text-gray-700">Durée</label>
          <select
            id="duree"
            className="form-select block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
            value={duree}
          ...
          <tbody>
            ...
            {users.map(user => (
              <tr key={user._id}>
                ...
                <td>{user.dateExpiration ? new Date(user.dateExpiration).toLocaleDateString() : 'N/A'}</td>
